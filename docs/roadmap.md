...
- [feito] `tests/test_text.c` (runner por fixtures: `.in/.base/.out/.title/.links/.warn`) e 19 casos golden em `tests/fixtures/html-to-text/` (título, blocos, entidades, refs numéricas C1/Windows-1252, refs NULL/surrogate/fora-de-faixa → U+FFFD, links resolvidos/relativos, script/style descartados, comentários, br/listas com marcadores (`-`/`N.`), whitespace, `<pre>` pre-formatado, e recuperação de malformado/unclosed).
